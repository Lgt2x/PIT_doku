...
def play():
    askPlayer = input("Do You wanna Start Playing ?: Yes or No")
    if askPlayer == "Yes":
        askPlayerModel = input("Do You wanna play with your own grid ?: Yes or No")
        if askPlayerModel == "Yes":
            NewSudoku = grid.SudokuGrid.from_stdin()
        elif askPlayerModel == "No":
            askFile = input("Saisir Un fichier Source :")
            askLine = input("Saisir la ligne génératrice de la grille:")
            NewSudoku = grid.SudokuGrid.from_file(askFile, int(askLine))
        else:
...
def run(Sudoku):
    while len(Sudoku.get_empty_pos()) != 0:
        ...
        if Solve == "Yes":
            Solver = solver.SudokuSolver(Sudoku)
            Solver.solve()
            print(Sudoku)
